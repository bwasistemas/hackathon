"""Use case: OCR → LLM analysis → publish result to diagram.result queue."""
import json
import logging
import os
import magic
...
from app.application.ports import LlmAnalyzerPort, ResultPublisherPort
...
class ProcessDiagramUploadUseCase:
    ...
    async def execute(self, upload_id: str, file_path: str) -> None:
        uid = str(upload_id)
        await self._publisher.publish_processing(uid)
        ...
        try:
            ...
            await self._publisher.publish_done(uid, payload)

        except Exception as e:
            logger.exception("Diagram processing failed for upload %s", uid)
            try:
                await self._publisher.publish_failed(uid, str(e))
            except Exception as pub_err:
                logger.error("Could not publish ERROR status: %s", pub_err)
        finally:
